/**********************************************************************
  ...
  Adapted by Eddy Luursema October 23 2019
  F1 decrease loc 3 speed with a mininum of -120
  F2 stop
  F3 decrease loc 3 speed with a maximum of 120
...
const int LEFTSQUAREBRACKET = 0x5B;
const int TILDE = 0x7E;
const int ASCIIZERO = 0x30;
const int ASCIININE = 0x39;
enum receiveStates {WAITESCAPE, WAITLEFTSQUARE, WAITFIRST, WAITSECOND, WAITTILDE};

receiveStates receiveState;
int locSpeed = 0;
int firstKey;
int secondKey;
char dccString[MAX_COMMAND_LENGTH + 1];         //command to send to DCC++ station
//////////////Eddy//////////////
...
void SerialCommand::init(volatile RegisterList *_mRegs, volatile RegisterList *_pRegs, CurrentMonitor *_mMonitor) {
  ...
  receiveState = WAITESCAPE;
} // SerialCommand:SerialCommand


//////////////Eddy//////////////
void SerialCommand::testFunctionKeys(char c) {
  switch (receiveState) {
    case WAITESCAPE:
      if (c == ESCAPE) {
        receiveState = WAITLEFTSQUARE;
      }
      break;
    case WAITLEFTSQUARE:
      if (c == LEFTSQUAREBRACKET) {
        receiveState = WAITFIRST;
      } else {
        receiveState = WAITESCAPE;
      }
      break;
    ...
    case WAITSECOND:
      ...
      break;
    case WAITTILDE:
      if (c == TILDE) { //we have received a coorect VT-100 command code
        if (firstKey == 1) {  //F1 to F8
          switch (secondKey) {
            case 1 :  //F1 pressed decrease speed
              if (locSpeed >= MINSPEED + SPEEDSTEP) {
                locSpeed -= SPEEDSTEP;
                sprintf(dccString, "t 1 3 %d %d", abs(locSpeed), (locSpeed > 0) ? 1 : 0);
              }
              break;
            case 2: //F2 pressed STOP
              locSpeed = 0;
              sprintf(dccString, "t 1 3 %d %d", abs(locSpeed), (locSpeed > 0) ? 1 : 0);
              break;
            case 3: //F3 pressed increase speed
              if (locSpeed <= MAXSPEED - SPEEDSTEP) {
                locSpeed += SPEEDSTEP;
                sprintf(dccString, "t 1 3 %d %d", abs(locSpeed), (locSpeed > 0) ? 1 : 0);
              }
              break;
            ...
            case 5:  //F5 POWER OFF
              ...
              break;
            case 7:  //F6 LIGHT ON
              sprintf(dccString, "f 3 144");
              break;
            case 8:  //F7 LIGHT OF
              sprintf(dccString, "f 3 128");
              break;
            case 9:  //F8 pressed set accessory adress 501
              sprintf(dccString, "a 126 0 1");
              break;
            default:
              sprintf(dccString, "");
          }
          ...
          if (firstKey == 2) {
            switch (secondKey) {
              case 0 :  //F9 pressed set accessory
                sprintf(dccString, "a 126 0 0");
                break;
              case 1 :  //F10 pressed set accessory
                sprintf(dccString, "a 126 1 1");
                break;
              case 3 :  //F11 pressed set accessory
                sprintf(dccString, "a 126 1 0");
                break;
              default:
                sprintf(dccString, "");
            }
